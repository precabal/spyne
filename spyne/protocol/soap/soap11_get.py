...
from spyne.const.http import HTTP_200, HTTP_405, HTTP_500
...
class Soap11_Get(XmlDocument):
    ...
    def __init__(self, *args, **kwargs):
        super(Soap11_Get, self).__init__(*args, **kwargs)
    ...
    def create_in_document(self, ctx, charset=None):
        if isinstance(ctx.transport, HttpTransportContext):
            ...
            if content_type is None or http_verb not in self.allowed_http_verbs:
                ctx.transport.resp_code = HTTP_405
                raise RequestNotAllowed(
                        "You must issue a POST or GET request with the Content-Type "
                        "header properly set.")
            if http_verb == 'GET':


                with open('spyne/spyne/const/soap_msg.txt', 'r') as myfile:
                    test_xml = myfile.read().replace('\n', '')
                ctx.transport.resp_code = HTTP_200
                ctx.in_document = _parse_xml_string(test_xml,
                    XMLParser(**self.parser_kwargs), charset)
                return
